...
class DynamicNetwork(Backbone):
    ...
    def forward(self, x, step_rate=0.0):
        ...
        for layer_index in range(len(self.cell_num_list)):
            ...
            for _cell_index in range(len(self.all_cell_type_list[layer_index])):
                if self.cal_flops:
                    cell_output, gate_weights_beta, cell_flops, \
                        cell_flops_real, trans_flops, trans_flops_real, gate_mask = \
            ...
            prev_gate_list.append(layer_gate)
            prev_gate_weight_list.append(layer_gate_weights)
        ...
        return final_out_dict, all_cell_flops, all_flops_real,prev_gate_list, prev_gate_weight_list
