...
def main(config):
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        loss = sum(loss for loss in loss_dict.values() if loss.requires_grad)
        loss.backward()
        ...
        if i_iter % 100 == 0 and i_iter != 0:
            print('iter = {0:6d}/{1:6d}, loss = {2:.3f}'.format(i_iter, num_iterations, loss))
...
'''
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if train_unlabeled:
            print(
                'iter = {0:6d}/{1:6d}, loss_l = {2:.3f}, loss_u = {3:.3f}'.format(i_iter, num_iterations, loss_l_value,
                                                                                  loss_u_value))
        else:
        ...
        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            mIoU, eval_loss = evaluate(model, dataset, ignore_label=ignore_label, input_size=(512, 1024),
                                       save_dir=checkpoint_dir)
    ...
    model.eval()
    mIoU, val_loss = evaluate(model, dataset, ignore_label=ignore_label, input_size=(512, 1024),
                              save_dir=checkpoint_dir)
