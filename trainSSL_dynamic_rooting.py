...
def main(config):
    ...
    # create network
    if config['model'] == 'dynamic_routing':
        model = DynamicNet4Seg(config)
    elif config['model'] == 'DeepLab':
        model = Res_Deeplab(num_classes=num_classes)
    '''
    # load pretrained parameters
    if config['pretrained'] is True and restore_from[:4] == 'http':
        saved_state_dict = model_zoo.load_url(restore_from)
    elif config['pretrained'] is True:
        saved_state_dict = torch.load(restore_from)
        '''
    if config['pretrained'] is True:
        saved_state_dict = torch.load(config['MODEL']['WEIGHTS'])
        new_params = model.state_dict().copy()
        for name, param in new_params.items():
            if name in saved_state_dict and param.size() == saved_state_dict[name].size():
                new_params[name].copy_(saved_state_dict[name])
        model.load_state_dict(new_params)
    # checkpointer = DetectionCheckpointer(model,config['MODEL']['OUTPUT_DIR'])
    # checkpointer.resume_or_load(config['MODEL']['WEIGHTS'], resume=True)

    # Initiate ema-model
    if train_unlabeled and config['model'] is 'DeepLab':
        ema_model = create_ema_model(model)
        ema_model.train()
        ema_model = ema_model.cuda()
    elif train_unlabeled and config['model'] is 'dynamic_routing':
        ema_model = model
    else:
        ema_model = None
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if config['model'] == 'dynamic_routing':
            pred = model(images, iter_rate)
        else:
            pre_out = model(images)
            pred = interp(pre_out)
        L_l = loss_calc(pred, labels)

        # Training loss for unlabeled data only
        if train_unlabeled:
            try:
            ...
            if config['model'] is 'dynamic_routing':
                logits_u_w = model(inputs_u_w, iter_rate)
            else:
                logits_u_w = interp(model(inputs_u_w))
            ...
            if config['model'] is 'dynamic_routing':
                logits_u_s = model(inputs_u_s, iter_rate)
            else:
        ...
        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            mIoU, eval_loss = evaluate(model, dataset, ignore_label=ignore_label, input_size=input_size,
    ...
    model.eval()
    mIoU, val_loss = evaluate(model, dataset, ignore_label=ignore_label, input_size=input_size,
                              save_dir=checkpoint_dir)
